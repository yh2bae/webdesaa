...
@section('head-title')
<title>Desa {{ $desa->nama_desa }} - Role</title>
@endsection
...
<div class="row">
    <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
        ...
        <a href="{{ route('role.create') }}" class="btn btn-sm btn-primary mt-3">+ Tambah</a>
        @endif  
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <form onsubmit="return confirm('Are you sure ?');"
                                action="{{ route('role.destroy', $s->id_role) }}"
                                method="POST">

                                @if(user_akses2('roles',Session()->get('level'))->posting ?? 0 =='1')
                                <a href="{{ route('role.access', $s->id_role) }}" title="Access" class="btn btn-success btn-sm">
                                    <i class="far fa-eye"></i>
                                </a>
                                @endif

                                @if(user_akses2('roles',Session()->get('level'))->update ?? 0 =='1')
                                <a href="{{ route('role.edit', Crypt::encryptString($s->id_role)) }}" title="Edit" class="btn btn-primary btn-sm">
                                    <i class="far fa-edit"></i>
                                </a>
                                @endif
                                
                                @if(user_akses2('roles',Session()->get('level'))->delete ?? 0 =='1')
